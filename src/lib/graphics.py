...
class GraphWin(tk.Canvas):
    ...
    def resize(self, width, height):
        """Resize the window (canvas) to a new width and height in pixels.
        Preserves current world coordinates if setCoords() was used."""
        self.__checkOpen()
        # Preserve old dimensions for coordinate recalculation
        old_w = self.width
        old_h = self.height

        # Update internal size and configure canvas
        self.width = int(width)
        self.height = int(height)
        ...
        # If coordinates were set, keep the same world view after resizing
        if self.trans:
            xlow = self.trans.xbase
            yhigh = self.trans.ybase
            xhigh = xlow + self.trans.xscale * (old_w - 1)
            ylow = yhigh - self.trans.yscale * (old_h - 1)
            self.setCoords(xlow, ylow, xhigh, yhigh)
